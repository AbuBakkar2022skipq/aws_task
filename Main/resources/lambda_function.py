...
def lambda_handler(event, context):
    ...
    item_count = 0
    dictionary=[{'transaction_Amount':0, 'transaction_Memo':'', 'date_Posted':''}]
    ...
    with conn.cursor() as cur:
        ...
        for i in range(len(response['data']['report'])):
            ins= 'insert into records (record_ID,parent_Account_Number,transaction_Amount,gL_Account_Name_Full,transaction_Memo,gL_Account_Name,debit_Amount,date_Posted,account_Type,credit_Amount,unique_Identifier) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)'
            ...
            unique_identifier=t_memo+t_amm+deb_amm+acc_type
            
            
            cur.execute(ins, (None, pan, t_amm, acc_n_full, t_memo, acc_n, deb_amm, d_posted, acc_type, cred_amm,unique_identifier))
        conn.commit()
